...
const Allpdfs = () => {
  ...
  return (
    <>
          ...
          {filterdData?.map((ele) => (
            <Grid item xs={12} sm={6} md={4} lg={3} key={ele._id}>
              ...
              <Card sx={{ margin: " 10px 20px", display: "flex", alignItems: "center", flexDirection: "column", color: "#fff", maxWidth: 345, backgroundColor: "unset", boxShadow: " 0 0 10px 1px rgba(255,255,255,0.5)", }}>
                ...
                <Typography variant="h5" sx={{ margin: "0", textDecoration: "1px solid primary" }}>
                  {ele?.teacher?.name}
                </Typography>
                ...
                <div style={{ width: "100%", display: "flex", justifyContent: "end", padding: "0 30px 10px", }}></div>
                <Preview open={modal.open} handleClose={handleClose} src={ele?.src} />
                <CopyNumber open={copyModal.open} handleCloseCopy={handleCloseCopy} pdf={ele?._id} />
              </Card>
            </Grid>
          ))}
        </Grid>
      ...
      <ConfirmationDialoge type={'pdf'} open={dialog.open} handleClose={handleCloseDialoge} id={dialog.id} />
      <AddPdf teacher={true} update={update.open} data={update.data} handleCloseUpdate={handleCloseUpdate} />
    </>
  )
}
